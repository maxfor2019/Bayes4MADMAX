...
signal(rhoa, ma, vsig, freq, ex, kwargs) = signal_counts_bin((freq./kwargs.scale_ω) .+kwargs.f_ref, ma*1e-6,rhoa, σ_v,ex) # 800 μs
...
likelihood = let d=collect(data[1]), e=data[2], f=fit_function, kwargs=kwarg_dict, ex=ex
    function logl(pars) 
        #println(pars)
        expectation = f(pars, d .* kwargs.scale_ω, ex, kwargs=kwargs) # 800 μs
