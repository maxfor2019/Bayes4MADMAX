...
function read_MADMAX_data(filepath, binsize)
...
MADMAX_data = read_MADMAX_data(joinpath("./data/Fake_Axion_Data/Data Set 2", "Het3_10K_0-15z_18-85GHz_20170413_160107_S04.smp"), 1);

DATA_DIM = size(MADMAX_data, 1);

data = MADMAX_data .* 1e23 ; # rescale power!

DATA_XLIM = [1.0, 50.8454]; # [MHz]
...
starting_point_a = randn(last(PARIDX).stop);

k = collect(0:(_GP_DIM)÷2 -1);

function sqrt_kernel(p)
    kernel_A_c, kernel_l_c = p[PARIDX.gp_hyper]
    kernel_A = 300.0*exp(kernel_A_c*0.9)*GP_GRAIN_FACTOR # determines amplitude and differences in amplitude of gaussian
    kernel_l = 0.1*exp(kernel_l_c/15)/(GP_GRAIN_FACTOR^0.3) # determines amplitude and differences in std
...
plot()
plot_kernel_model(starting_point_a, 10)
...
plot()
plot_kernel_matrix(starting_point_a)
...
plot()
plot_kernel_prior_samples(200, 10)

plot()
plot_data()
plot_mean(starting_point_a, "starting point"; plot_args=(;color=:darkorange2))
plot_mgvi_samples(produce_posterior_samples(starting_point_a, 6))

plot()
plot_kernel_model(starting_point_a, 10; plot_args=(;label="kernel model"))
plot_kernel_mgvi_samples(produce_posterior_samples(starting_point, 6), 10)
...
plot()
plot_kernel_model(first_iteration.result, 10; plot_args=(;label="kernel model"))
plot_kernel_mgvi_samples(first_iteration.samples, 10)
